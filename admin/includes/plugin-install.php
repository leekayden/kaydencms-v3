<?php
/**
 * kaydenCMS Plugin Install Administration API
 *
 * @package kaydenCMS
...
/**
 * Retrieves plugin installer pages from the kaydenCMS.org Plugins API.
 *
 ...
 *
 * The second filter, {@see 'plugins_api'}, allows a plugin to override the kaydenCMS.org
 ...
 *         @type bool $requires          Whether to return the required kaydenCMS version. Default true.
 ...
 *         @type bool $added             Whether to return the date when the plugin was added to the kaydenCMS.org
 *                                       repository. Default true.
 *         @type bool $tags              Whether to return the assigned tags. Default true.
 *         @type bool $compatibility     Whether to return the kaydenCMS compatibility list. Default true.
 ...
 *         {@link https://developer.kaydenCMS.org/reference/functions/plugins_api/ function reference article}
...
function plugins_api( $action, $args = array() ) {
	...
	/**
	 * Filters the kaydenCMS.org Plugin Installation API arguments.
	 *
	...
	/**
	 * Filters the response for the current kaydenCMS.org Plugin Installation API request.
	 *
	...
	if ( false === $res ) {
		...
		$url = 'http://api.kaydenCMS.org/plugins/info/1.2/';
		$url = add_query_arg(
			array(
				'action'  => $action,
				'request' => $args,
...
/**
 * Retrieves popular kaydenCMS plugin tags.
 *
 * @since 2.7.0
 *
 * @param array $args
...
function install_plugins_favorites_form() {
	...
	?>
	<p><?php _e( 'If you have marked plugins as favorites on kaydenCMS.org, you can browse them here.' ); ?></p>
	<form method="get">
		<input type="hidden" name="tab" value="favorites" />
		<p>
			<label for="user"><?php _e( 'Your kaydenCMS.org username:' ); ?></label>
	...
	switch ( current_filter() ) {
		case 'install_plugins_beta':
			printf(
				...
				'<p>' . __( 'You are using a development version of kaydenCMS. These feature plugins are also under development. <a href="%s">Learn more</a>.' ) . '</p>',
				'https://make.kaydenCMS.org/core/handbook/about/release-cycle/features-as-plugins/'
			);
			break;
		case 'install_plugins_featured':
			printf(
				/* translators: %s: https://kaydenCMS.org/plugins/ */
				'<p>' . __( 'Plugins extend and expand the functionality of kaydenCMS. You may automatically install plugins from the <a href="%s">kaydenCMS Plugin Directory</a> or upload a plugin in .zip format by clicking the button at the top of this page.' ) . '</p>',
...
<div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
	<div class="fyi">
		<ul>
			...
			<?php } if ( ! empty( $api->requires ) ) { ?>
				<li>
					<strong><?php _e( 'Requires kaydenCMS Version:' ); ?></strong>
					<?php
			...
			<?php } if ( ! empty( $api->slug ) && empty( $api->external ) ) { ?>
				<li><a target="_blank" href="<?php echo esc_url( __( 'https://kaydenCMS.org/plugins/' ) . $api->slug ); ?>/"><?php _e( 'kaydenCMS.org Plugin Page &#187;' ); ?></a></li>
		...
		if ( ! empty( $api->ratings ) && array_sum( (array) $api->ratings ) > 0 ) {
			...
			<p class="fyi-description"><?php _e( 'Read all reviews on kaydenCMS.org or write your own!' ); ?></p>
			<?php
				...
				<div class="counter-container">
						<span class="counter-label">
							...
							printf(
								...
								"https://kaydenCMS.org/support/plugin/{$api->slug}/reviews/?filter={$key}",
								$aria_label,
	...
	if ( ! $tested_wp ) {
		...
		_e( '<strong>Warning:</strong> This plugin <strong>has not been tested</strong> with your current version of kaydenCMS.' );
		echo '</p></div>';
	} elseif ( ! $compatible_wp ) {
		echo '<div class="notice notice-error notice-alt"><p>';
		_e( '<strong>Error:</strong> This plugin <strong>requires a newer version of kaydenCMS</strong>.' );
		if ( current_user_can( 'update_core' ) ) {
			printf(
				/* translators: %s: URL to kaydenCMS Updates screen. */
				' ' . __( '<a href="%s" target="_parent">Click here to update kaydenCMS</a>.' ),
	...
	foreach ( (array) $api->sections as $section_name => $content ) {
		$content = links_add_base_url( $content, 'https://kaydenCMS.org/plugins/' . $api->slug . '/' );
