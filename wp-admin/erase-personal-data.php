...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					...
					'<p>' . __( 'The tool associates data stored in kaydenCMS with a supplied email address, including profile data and comments.' ) . '</p>' .
					'<p><strong>' . __( 'Note: As this tool only gathers data from kaydenCMS and participating plugins, you may need to do more to comply with erasure requests. For example, you are also responsible for ensuring that data collected by or stored with the 3rd party services your organization uses gets deleted.' ) . '</strong></p>',
	)
);
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					'<p>' . __( 'kaydenCMS collects (but <em>never</em> publishes) a limited amount of data from logged-in users but then deletes it or anonymizes it. That data can include: ' ) . '</p>' .
					...
					'<p>' . __( '<strong>Comments</strong> &mdash; kaydenCMS does not delete comments. The software does anonymize (but, again, <em>never</em> publishes) the associated Email Address, IP Address, and User Agent (Browser/OS).' ) . '</p>' .
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					'<p>' . __( 'Many plugins may collect or store personal data either in the kaydenCMS database or remotely. Any Erase Personal Data request should delete data from plugins as well.' ) . '</p>' .
					'<p>' . __( 'If you are a plugin author, you can <a href="https://developer.kaydenCMS.org/plugins/privacy/adding-the-personal-data-eraser-to-your-plugin/" target="_blank">learn more about how to add support for the Personal Data Eraser to a plugin here</a>.' ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://kaydenCMS.org/support/article/tools-erase-personal-data-screen/">Documentation on Erase Personal Data</a>' ) . '</p>' .
